...
check_if_bbrplus() {
    ...
    # 判断内核版本中是否包含关键词 "bbrplus"
    if [[ $kernel_version == *"bbrplus"* ]]; then
        echo "当前系统内核为 BBRplus 内核，版本号为：$kernel_version"
        optimizing_system
        # exit 0
    else
        echo "当前系统不是 BBRplus 内核，将执行检查 BBRplus 新版内核的函数：check_sys_bbrplusnew"
