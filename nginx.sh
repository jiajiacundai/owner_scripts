...
# --sbin-path=$PWD                        Name of the NGINX executable file, which is used only during installation. Default: /sbin/nginx.
#                                         NGINX可执行文件的名称, 仅在安装期间使用。默认值:  / sbin / nginx
# --conf-path=$PWD/nginx.conf             Name of the NGINX configuration file. You can, however, always override this value at startup by specifying a different file with the -c  option on the nginx command line. Default: conf/nginx.conf.
#                                         NGINX配置文件的名称。但是, 您可以通过在nginx命令行上使用-c 选项指定其他文件来始终在启动时覆盖此值。默认值: conf/nginx.conf
# --pid-path=$PWD/nginx.pid               Name of the nginx.pid file, which stores the process ID of the nginx master process. After installation, the path to the filename can be changed with the pid directive in the NGINX configuration file. Default: /logs/nginx.pid.
#                                         nginx.pid文件的名称, 用于存储nginx主进程的进程ID。安装后, 可以使用NGINX配置文件中的pid指令更改文件名的路径。默认值: /logs/nginx.pid
# --error-log-path=$PWD/logs/error.log    Name of the primary log file for errors, warnings, and diagnostic data. After installation, the filename can be changed with the the error_log directive in the NGINX configuration file. Default: /logs/error.log.
#                                         错误, 警告和诊断数据的主日志文件的名称。安装后, 可以使用NGINX配置文件中的error_log指令更改文件名。默认值: /logs/error.log
# --http-log-path=$PWD/logs/access.log    Name of the primary log file for requests to the HTTP server. After installation, the filename can always be changed with the access_log directive in the NGINX configuration file. Default: /logs/access.log.
#                                         HTTP服务器请求的主日志文件的名称。安装后, 始终可以使用NGINX配置文件中的access_log指令更改文件名。默认值: /logs/access.log
