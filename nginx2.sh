...
# 安装Nginx
read -p "是否安装公共 nginx (y/n)? " _INSTALL_FROM_PACKAGE
...
# 读取用户输入是否编译依赖库
read -p "是否需要编译依赖库？(y/n): " COMPILE

if [[ "$COMPILE" == "y" ]]; then
    # 根据系统类型安装必要的编译依赖
    if [[ "$OS" == "centos" ]]; then
        yum install -y git make gcc sudo libxml2 libxml2-devel libxslt libxslt-devel wget zlib-devel
    elif [[ "$OS" == "ubuntu" || "$OS" == "debian" ]]; then
        apt update
        apt install -y git make gcc sudo libxml2 libxml2-dev libxslt1-dev wget zlib1g-dev  
    else
        echo "操作系统不受支持"
        exit 1
    fi
    ...
    # 添加编译库
    CONFIGURE_ARGS="$CONFIGURE_ARGS --with-pcre=/usr/local/src/pcre-${pcre_version} --with-openssl=/usr/local/src/openssl-${OPENSSL_VERSION}"
else
    # 不编译依赖，直接使用系统包管理器安装依赖
    if [[ "$OS" == "centos" ]]; then
